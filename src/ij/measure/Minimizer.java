...
public class Minimizer {
    ...
    private CurveFitter minimizeFunction;   // target function to minimize
     ...
     *                      of the function.
     */
    public void setFunction(CurveFitter function, int numParams) {
        if (maxIter<=0) {
            maxIter = ITER_FACTOR*numParams*numParams;
            if (maxRestarts > 0)
                maxIter *= 2;
        }
        this.minimizeFunction = function;
    ...
    private void evaluate(double[] vertex) {
        vertex[numParams] = minimizeFunction.userFunction(vertex, 0);
    }
